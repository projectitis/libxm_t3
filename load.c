	...
	for(uint8_t i = 0; i < ctx->module.length; ++i) {
		if(ctx->module.pattern_table[i] >= ctx->module.num_patterns) {
			if(i+1 == ctx->module.length && ctx->module.length > 1) {
				...
				--ctx->module.length;
				#ifdef XM_DEBUG
					sprintf( xm_debugstr, "trimming invalid POT at pos %X", i);
					xm_stdout( xm_debugstr );
				#endif
			} else {
				#ifdef XM_DEBUG
					sprintf( xm_debugstr, "module has invalid POT, pos %X references nonexistent pattern %X", i, ctx->module.pattern_table[i]);
					xm_stdout( xm_debugstr );
				#endif
				return 1;
			}
